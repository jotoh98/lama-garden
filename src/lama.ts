...
export const updateLama = (lama: Lama, _frame: number, delta: number) => {
  ...
  switch (lama.state.type) {
    ...
    case "MOVING":
      ...
      lama.transform.y = Math.floor(Date.now() / 500) % 3 === 0 ? -1 : 0
...
const lamaColor = new Color("#c58e53")
const lamaEyeColor = new Color("#607be5")
...
const renderLamaIcon = (ctx: CanvasRenderingContext2D, lama: Lama, frame: number) => {
  ...
  ctx.lineWidth = 0.2
  ctx.fillStyle = cycleDayNightColors(lamaColor, lamaColor.darken(0.4))
  ctx.fill(LAMA_PATH)
  ctx.strokeStyle = "#000"
  ctx.stroke(LAMA_PATH)

  ctx.fillStyle = cycleDayNightColors(lamaEyeColor, lamaEyeColor.darken(0.4))
